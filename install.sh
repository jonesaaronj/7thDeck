...
while true; do
  ...
  if [ "$PROTON" = "NONE" ]; then
    ...
    nohup $(launchSteam) steam://install/1493710 &>/dev/null &
...
pkill -9 steam
cp $(getSteamRoot)/config/config.vdf $(getSteamRoot)/config/config.vdf.bak
perl -0777 -i -pe 's/"CompatToolMapping"\n\s+{/"CompatToolMapping"\n\t\t\t\t{\n\t\t\t\t\t"39140"\n\t\t\t\t\t{\n\t\t\t\t\t\t"name"\t\t"proton_experimental"\n\t\t\t\t\t\t"config"\t\t""\n\t\t\t\t\t\t"priority"\t\t"250"\n\t\t\t\t\t}/gs' \
  $(getSteamRoot)/config/config.vdf
while pgrep "steam" >/dev/null; do sleep 1; done
[ "${WINEPATH}" = */compatdata/39140/pfx ] && rm -rf "${WINEPATH%/pfx}"/*
echo "Sign into the Steam account that owns FF7 if prompted."
nohup $(launchSteam) steam://rungameid/39140 &>/dev/null &
...
while true; do
  INSTALL_PATH=$(promptDirectory "Select 7th Heaven Install Folder") || {
...
if find $INSTALL_PATH -mindepth 1 -maxdepth 1 | read; then
  echo "Skipping install of 7th Heaven"
else
  ...
  STEAM_COMPAT_APP_ID=39140 STEAM_COMPAT_DATA_PATH="${WINEPATH%/pfx}" \
    STEAM_COMPAT_CLIENT_INSTALL_PATH=$(readlink -f "$(getSteamRoot)") \
