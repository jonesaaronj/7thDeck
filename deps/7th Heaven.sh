...
if [ $IS_STEAMOS = true ]; then
  export q=${HOME}/.steam/steam/steamapps/compatdata/39140
else
...
export STEAM_COMPAT_CLIENT_INSTALL_PATH=$(readlink -f "$(getSteamRoot)")
...
[ ! -d "$STEAM_COMPAT_DATA_PATH" ] && {
  promptUser "FF7 prefix not found!"
  exit 1
}
[ "$RUNTIME" = "NONE" ] && {
  promptUser "Steam Linux Runtime not found!"
  exit 1
}
[ "$PROTON" = "NONE" ] && {
  promptUser "Proton not found!"
  exit 1
}
